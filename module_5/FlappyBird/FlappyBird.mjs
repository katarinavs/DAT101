  ...
  switch (GameProps.status) {
    ...
    case EGameStatus.gameOver:
      ...
      if (delBaitIndex >= 0) {
        ...
        GameProps.menu.incScore(10); // Poeng for å spise sommerfugl
      }
      break;
...
function spawnBait() {
  ...
  // Genererer nye baits hvert 0.5 til 1 i sekund med step på på 0.1
  if(GameProps.status === EGameStatus.playing) { // Får baits til å forsvinne når man dør
...
function setDayNight() {
  if (rbDayNight[0].checked) {
    GameProps.background.index = 0; // endrer bakgrunn til dag
    GameProps.dayTime = true;
    console.log("Day time");
  } else {
    GameProps.dayTime = false;
    GameProps.background.index = 1; // endrer bakgrunn til natt
