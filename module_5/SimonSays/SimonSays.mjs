...
export const gameProps = {
  ...
  activeButton: null, // Ingen knapp er aktiv i starten av spillet.
...
function setDisableButtons(aDisabled){
  for (let i = 0; i < gameProps.ColorButtons.length; i++){
...
function setMouseUp() {
  ...
  if(gameProps.seqIndex < gameProps.sequence.length - 1){
    ...
    // Her er den site knappen i sekvensen.
    gameProps.activeButton.onMouseUp();
...
export function spawnSequence() {
  ...
  if(gameProps.GameSpeed > 200){
    gameProps.GameSpeed -= 200; // funker ikke på 100?
  } else { 
